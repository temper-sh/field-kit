#!/usr/bin/env bash
#
# Machine report — will this Mac run the local-ai-setup stack, and in which
# bucket? Prints one copy-pasteable block; send the whole block back.
#
#   scripts/machine-report.sh
#
# SELF-CONTAINED ON PURPOSE: no Homebrew, no yq, no repo checkout — this one
# file can be sent to a friend and runs on a stock Mac (bash 3.2, read-only
# sysctls, nothing installed, nothing written). That is also why the
# constants below are duplicated from scripts/gpu-budget.sh and the README
# bucket table instead of sourced: keep the three in sync by hand.
#
# This is stage 0 (preflight) of the field kit: machine facts and the
# bucket verdict, deciding whether the install probe is worth running at
# all. The probe itself — install trial, serve trial, fit soak, perf — is
# the separate field-kit repo (docs/PLAN.md §9). THIS copy is canonical
# (check 20 tests it); a distribution copy ships at the top of
# github.com/temper-sh/field-kit so friends can run stage 0 before cloning
# anything — update it by hand when this file changes (the kit's tests pin
# the wired-limit formula, so formula drift fails loudly there).
#
